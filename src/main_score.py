...
def calc_performance_odin(root: str, trainset_name: str) -> Tuple[dict, dict, list]:
    ...
    for ood_root in tqdm(list(ood_roots), desc="odin_pnml: ood"):
        ...
        # Max prob
        performance_dict = calc_max_prob_pref(
            ood_name, ind_prob_path, ood_dict["prob_path"]
        )
        odin_performance_list.append(performance_dict)

        # Compute pNML
        ind_features, _, ind_probs = load_ind_products(
            ood_root + "_pNML", trainset_name
        )
        ...
        regrets_ind, pnml_prediction = calc_regret_on_set(
            ind_features, ind_probs, p_parallel, p_bot
        )
...
def calc_performance_gram(root: str, trainset_name: str) -> Tuple[dict, dict]:
    # Initialize output
    ...
    for ood_name, ood_dict in tqdm(ood_dicts.items(), desc="gram: ood"):
        ...
        for seed in range(1, 11):
            ...
            # Calc metric pnml
            performance_dict = calc_gram_pref(
                ood_name, ind_deviations_norm, ood_deviations_norm
            )
            ...
            ind_deviations_norm = np.sqrt(
                (deviations_ind_test / dev_norm_std).mean(axis=1, keepdims=True)
            )
            ood_deviations_norm = np.sqrt(
                (deviations_ood / dev_norm_std).mean(axis=1, keepdims=True)
            )
            regrets_ind, pnml_prediction = calc_regret_on_set(
                ind_features[test_indices] * ind_deviations_norm,
...
def calc_performance_baseline(root: str, trainset_name: str) -> Tuple[dict, dict]:
    # Initialize output
    ...
    regrets_ind, pnml_prediction = calc_regret_on_set(
        features_ind, probs_ind, p_parallel, p_bot
    )
...
def eval_single_model_performance(roots_i: str):
    ...
    metric_df_dict = {
        metric_name: pd.concat(metric_list, axis=1)
        for metric_name, metric_list in metric_dfs.items()
    }
...
if __name__ == "__main__":
    ...
    with futures.ProcessPoolExecutor(num_workers) as pool:
        ...
        for df_dicts in pool.map(eval_single_model_performance, roots):
            ...
            print(
                "[{}/{}].in {:.2f} sec".format(
                    finished, len(roots) - 1, time.time() - t1
                )
            )
            finished += 1
            t1 = time.time()
    ...
    df_dict = {
        metric_name: pd.concat(method_list, axis=0).round(2)
        for metric_name, method_list in model_result_dict.items()
    }
