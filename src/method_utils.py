...
def execute_baseline(
    cfg, lit_model_h: LitBaseline, trainer, loaders_dict
    ...
    for i, (ood_name, ood_dataloader) in enumerate(loaders_dict.items()):
        ...
        logger.info(
            "[{}/{}] {} as ood. AUROC [baseline pnml]=[{:.2f} {:.2f}] in {:.1f} sec".format(
                i,
                len(loaders_dict) - 1,
                ood_name,
                baseline_res["AUROC"],
...
def execute_odin(
    cfg, lit_model_h: LitOdin, trainer, loaders_dict
) -> Tuple[pd.DataFrame, pd.DataFrame]:
    # Eval training set (create (x^T x)^{-1} matrix)
    ...
    for epsilon_num, epsilon in enumerate(epsilons[::-1]):
        ...
        for temperature_num, temperature in enumerate(temperatures):
            ...
            for i, (ood_name, ood_dataloader) in enumerate(loaders_dict.items()):
                ...
                lit_model_h.set_ood(ood_name)
                trainer.test(
                    test_dataloaders=ood_dataloader, ckpt_path=None, verbose=False
                )
                ...
                results_dict[epsilon][temperature] = {
                    "baseline": baseline_results,
                    "pnml": pnml_results,
                }

    # Return Dataframes
    baseline_best_results, pnml_best_results = get_best_odin_results(
        loaders_dict.keys(), results_dict
    )
    return pd.DataFrame(baseline_best_results), pd.DataFrame(pnml_best_results)


def get_best_odin_results(
    ood_names: list, results_dict: dict
) -> Tuple[pd.DataFrame, pd.DataFrame]:
                ...
                for baseline_ood_res in epsilon_temperature_dict["baseline"]:
                    if baseline_ood_res["ood_name"] == ood_name:
                        ...
                        if auroc > baseline_best_auroc:
                            logger.info(
                                f"{ood_name}: baseline best auroc: {pnml_best_auroc}"
                            )
                            baseline_ood_best_results = baseline_ood_res
                            baseline_best_auroc = auroc
                            baseline_best_temperature, baseline_best_epsilon = (
                                temperature,
                                epsilon,
                            )
                ...
                for pnml_ood_res in epsilon_temperature_dict["pnml"]:
                    if pnml_ood_res["ood_name"] == ood_name:
                        ...
                        if auroc > pnml_best_auroc:
                            logger.info(
                                f"{ood_name}: pnml best auroc: {pnml_best_auroc}"
                            )
                            pnml_ood_best_results = pnml_ood_res
                            pnml_best_auroc = auroc
                            pnml_best_temperature, pnml_best_epsilon = (
                                temperature,
                                epsilon,
                            )
        logger.info(ood_name)
        logger.info(
            "baseline: [AUROC epsilon temperature]=[{:.2f} {} {}]".format(
                baseline_best_auroc, baseline_best_temperature, baseline_best_epsilon
            )
        )
        logger.info(
            "pnml: [AUROC epsilon temperature]=[{:.2f} {} {}]".format(
