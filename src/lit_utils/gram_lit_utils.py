...
from score_utils import (
    calc_metrics_transformed,
    split_val_test_idxs,
    compute_list_of_dict_mean,
)
...
class LitGram(LitBaseline):
    ...
    def training_step(self, batch, batch_idx):
        ...
        gram_mins, gram_maxs = compute_minmaxs_train(
            gram_features_batch, output["probs"]
        )
        ...
        for c in range(num_labels):
            for l in range(num_layers):
                ...
                mins_bathces = [
                    ...
                    for gram_mins_batch in gram_mins_batches
        ...
        for c in range(num_labels):
            for l in range(num_layers):
                ...
                maxs_batches = [
                    ...
                    for gram_maxs_batch in gram_maxs_batches
    ...
    def test_step(self, batch, batch_idx):
        ...
        deviations = compute_deviations(
            gram_features_batch, output["probs"], self.gram_mins, self.gram_maxs
        )
    ...
    def test_epoch_end(self, outputs):
        ...
        if self.is_ind:
            ...
            for seed in range(1, 1 + self.num_evaluations):
                # Split
                test_indices, val_indices = split_val_test_idxs(
                    len(self.ind_deviations), seed
                )
                ...
                baseline_res = self.execute_score_gram(
                    ind_deviations_val, ind_deviations_test, ood_deviations
                )
                pnml_res = self.execute_score_pnml_gram(
                    ind_deviations_val,
                    ind_deviations_test,
                    ood_deviations,
                    ind_features_test,
                    ood_features,
                    ind_probs_normalized_test,
                    ood_probs_normalized,
                )
    ...
    def execute_score_gram(
        self, ind_deviations_val, ind_deviations_test, ood_deviations
    ):
        ...
        baseline_res = calc_metrics_transformed(
            -ind_deviations_norm.cpu().numpy(), -ood_deviations_norm.cpu().numpy()
        )

        return baseline_res

    def execute_score_pnml_gram(
        self,
        ind_deviations_val,
        ind_deviations_test,
        ood_deviations,
        ...
        ind_deviations_norm = torch.sqrt(
            (ind_deviations_test / dev_norm_std).mean(dim=1, keepdims=True)
        )
        ood_deviations_norm = torch.sqrt(
            (ood_deviations / dev_norm_std).mean(dim=1, keepdims=True)
        )

        ind_regrets = (
            self.calc_regrets(
                ind_features_test * ind_deviations_norm, ind_probs_normalized_test
            )
            .cpu()
            .numpy()
        )
        ood_regrets = (
            self.calc_regrets(ood_features * ood_deviations_norm, ood_probs_normalized)
            .cpu()
            .numpy()
        )
...
def compute_deviations(gram_feature_layers, probs, gram_mins, gram_maxs):
    ...
    for c in predictions_unique:
        ...
        gram_features_per_class = [
            gram_feature_layer[class_idxs] for gram_feature_layer in gram_feature_layers
        ]
        max_probs_c = max_probs[predictions == c]

        deviations_c = get_deviations(
            gram_features_per_class, mins=gram_mins[c], maxs=gram_maxs[c]
        )
